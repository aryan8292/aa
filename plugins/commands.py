...
from Ashu.util.file_properties import get_name, get_hash, get_media_file_size
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [
                ...
                [
                    # InlineKeyboardButton(
                    #     "Eᴀʀɴ Mᴏɴᴇʏ 💸", callback_data="shortlink_info"
                    # ),
                    InlineKeyboardButton("ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ", url=GRP_LNK),
                ],
                [
        ...
        if CLONE_MODE == True:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "🤖 Cʀᴇᴀᴛᴇ Yᴏᴜʀ Oᴡɴ Cʟᴏɴᴇ Bᴏᴛ 🤖", callback_data="clone"
                    )
                ]
            )
        reply_markup = InlineKeyboardMarkup(buttons)
        m = await message.reply_sticker(
            "CAACAgUAAxkBAAECe2hnVyMwbEPJBoyClDXDd3kKcRR_WwACNwEAAq2_gTbG7srpndERQB4E"
        )
            ...
            if REQUEST_TO_JOIN_MODE == True:
                ...
                try:
                    ...
                    btn.append(
                        [
                            InlineKeyboardButton(
                                "↻ Tʀʏ Aɢᴀɪɴ", callback_data=f"checksub#{kk}#{file_id}"
                            )
                        ]
                    )
                except (IndexError, ValueError):
    ...
    if data.split("-", 1)[0] == "ASHU":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            ...
            await client.send_message(
                chat_id=user_id,
                text="<b>{} start the bot with your referral link\n\nTotal Referals - {}</b>".format(
                    message.from_user.mention, num_referrals
                ),
            )
            if num_referrals == REFERAL_COUNT:
                ...
                if seconds > 0:
                    ...
                    await client.send_message(
                        chat_id=user_id,
                        text="<b>You Have Successfully Completed Total Referal.\n\nYou Added In Premium For {}</b>".format(
    ...
    if data.split("-", 1)[0] == "BATCH":
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [
                        ...
                        [InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url=OWNER_USERNAME)],
                        [
                            InlineKeyboardButton(
                                "🚀 Fast Download 🚀", url=download
                            ),  # we download Link
        ...
        if str(message.from_user.id) != str(userid):
            return await message.reply_text(
                text="<b>Invalid link or Expired link !</b>", protect_content=True
            )
    ...
    if data.startswith("sendfiles"):
        ...
        g = await get_shortlink(
            chat_id, f"https://telegram.me/{temp.U_NAME}?start=allfiles_{file_id}"
        )
        k = await client.send_message(
            chat_id=message.from_user.id,
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            "⁉️ Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ ⁉️", url=await get_tutorial(chat_id)
                        )
                    ],
                ]
            ),
        ...
        files = files_[0]
        g = await get_shortlink(
            chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}"
        )
        k = await client.send_message(
            chat_id=user,
    ...
    if not files_:
        ...
        try:
            ...
            await msg.edit_caption(
                caption=f_caption, reply_markup=InlineKeyboardMarkup(button)
            )
            btn = [
                [
                    InlineKeyboardButton(
...
async def settings(client, message):
    ...
    if settings is not None:
        btn = [
            [
                InlineKeyboardButton(
                    "Oᴘᴇɴ Hᴇʀᴇ ↓", callback_data=f"opnsetgrp#{grp_id}"
                ),
                InlineKeyboardButton(
                    "Oᴘᴇɴ Iɴ PM ⇲", callback_data=f"opnsetpm#{grp_id}"
                ),
            ]
        ]
            ...
            if REQST_CHANNEL is not None:
                btn = [
                    [
                        ...
                        InlineKeyboardButton(
                            "Show Options", callback_data=f"show_option#{reporter}"
                        ),
                    ]
                ]
...
async def send_msg(bot, message):
    if message.reply_to_message:
        ...
        try:
            ...
            if success:
                await message.reply_text(
                    f"<b>Your message has been successfully send to {user.mention}.</b>"
                )
            else:
                await message.reply_text(
                    "<b>This user didn't started this bot yet !</b>"
                )
        except Exception as e:
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [
        [
            InlineKeyboardButton(
                "Yes, Continue !", callback_data=f"killfilesdq#{keyword}"
            )
        ],
...
async def settutorial(bot, message):
    ...
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(
            "This Command Work Only in group\n\nTry it in your own group"
        )
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        grpid = message.chat.id
        title = message.chat.title
    ...
    if len(message.command) == 1:
        ...
        return await message.reply(
            "<b>You entered Incorrect Format\n\nFormat: /set_tutorial your tutorial link</b>"
        )
...
async def stop_button(bot, message):
    msg = await bot.send_message(
        text="**🔄 𝙿𝚁𝙾𝙲𝙴𝚂𝚂𝙴𝚂 𝚂𝚃𝙾𝙿𝙴𝙳. 𝙱𝙾𝚃 𝙸𝚂 𝚁𝙴𝚂𝚃𝙰𝚁𝚃𝙸𝙽𝙶...**", chat_id=message.chat.id
    )
...
async def nofsub(client, message):
    ...
    if not userid:
        return await message.reply(
            f"<b>You are anonymous admin. Turn off anonymous admin and try again this command</b>"
        )
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        return await message.reply_text(
            "<b>This Command Work Only in group\n\nTry it in your own group</b>"
        )
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        grpid = message.chat.id
        title = message.chat.title
    ...
    await message.reply_text(
        f"<b>Successfully removed force subscribe from {title}.</b>"
    )
...
async def fsub(client, message):
    ...
    await message.reply_text(
        f"<b>Successfully set force channels for {title} to\n\n{channels}\n\nYou can remove it by /nofsub.</b>"
    )
...
async def plans_cmd_handler(client, message):
    ...
    btn = [
        [
            InlineKeyboardButton(
                "ꜱᴇɴᴅ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴘᴛ 🧾", url=f"https://t.me/{OWNER_USERNAME}"
            )
        ],
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):
        ...
        await message.reply_text(
            f"**😢 You Don't Have Any Premium Subscription.\n\n Check Out Our Premium /plan**",
            reply_markup=reply_markup,
        )
