...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ꜰɪʟᴇs ↭", callback_data=f"fy#homepage#{key}"
            )
        ]
    )
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        ...
        btn.append(
            [
                InlineKeyboardButton(
                    text="ɴᴏ ᴍᴏʀᴇ ᴘᴀɢᴇs ᴀᴠᴀɪʟᴀʙʟᴇ", callback_data="pages"
                )
            ]
        )
...
async def episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ꜰɪʟᴇs ↭", callback_data=f"fe#homepage#{key}"
            )
        ]
    )
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ꜰɪʟᴇs ​↭", callback_data=f"fl#homepage#{key}"
            )
        ]
    )
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append(
        [
            InlineKeyboardButton(
                text="↭ ʙᴀᴄᴋ ᴛᴏ ꜰɪʟᴇs ​↭", callback_data=f"next_{req}_{key}_{offset}"
            )
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings["is_shortlink"] and not await db.has_premium_access(
                ...
                if SHORTLINK_MODE == True:
                    ...
                    await query.answer(
                        url=f"https://telegram.me/{temp.U_NAME}?start=allfiles_{key}"
                    )
            elif settings["is_shortlink"] and await db.has_premium_access(
                query.from_user.id
            ):
        ...
        btn2 = [
            ...
            [InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url=f"{GRP_LNK}")],
        ]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            await query.answer(
                f"☣something went wrong sweetheart\n\n{e}", show_alert=True
            )
            return
        ...
        buttons = [
            [
                InlineKeyboardButton("ᴄᴏᴍᴍᴀɴᴅs ᴀɴᴅ ᴜsᴀɢᴇ", callback_data="allcmd"),
            ],
            # [
        ...
        buttons = [
            # [
            #     InlineKeyboardButton("Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ", url=SUPPORT_URL),
            #     InlineKeyboardButton(
            #         "Sᴏᴜʀᴄᴇ Cᴏᴅᴇ", url="https://t.me/AshutoshGoswami24"
            #     ),
            # ],
            [
                InlineKeyboardButton("ʜᴏᴍᴇ", callback_data="start")
        ...
        buttons = [[InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="allcmd")]]
        await client.edit_message_media(
            query.message.chat.id,
            query.message.id,
            InputMediaPhoto(random.choice(PICS)),
    ...
    #         [
    #             InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
    #             InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url=OWNER_USERNAME),
    #         ]
    #     ]
    #     await client.edit_message_media(
    #         query.message.chat.id,
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        vj_ai_msg = await reply_msg.edit_text(
            "<b><i>Advance Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>"
        )
        movienamelist = []
        movienamelist += [movie.get("title") for movie in movies]
        for Ashux in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(Ashux[0]):
                await auto_filter(client, Ashux, msg, reply_msg, vj_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [
            [
